...
export async function runJudgeAgent(
  ...
  try {
    ...
    const model = getModelForStage('judge', routing, strategy.judgeModel || strategy.defaultModel);
    const { data } = await callLLMJson<JudgeOutput>(prompt, undefined, model);
    ...
    console.error(`[Judge] Failed for ${marketId}:`, errorMsg);
    ...
    return {
      ...
      freshness: 0.5,
      catalystTiming: 'NONE',
      skipReason: `Judge agent failed: ${errorMsg}`,
    };
  }
}